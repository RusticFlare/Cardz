...
public class CardAssert extends AbstractAssert<CardAssert, Card> {
	...
	public CardAssert hasValue(Value expected) {
		if (actual.getValue() != expected)
			failWithMessage("Expected value <%s>, but was <%s>", expected.name(), actual.getValue().name());
		return myself;
	}
	
	public CardAssert hasSuit(Suit expected) {
		if (actual.getSuit() != expected)
			failWithMessage("Expected suit <%s>, but was <%s>", expected.name(), actual.getValue().name());
		return myself;
	}
